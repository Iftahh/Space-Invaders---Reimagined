...
updateAliens = function(dt) {
	...
	each(aliens, function(alien,ind) {
		...
		if (alien.life <= 0) {
			...
			if (alien.y > water_y+10*CELL_SIZE) {
				gameOver();
				OffsetY = alien.y - HEIGHT/2;
				OffsetX = alien.x - HEIGHT/2;
			}
			var radius = CELL_SIZE*10,
			dx = Player.pos.x - alien.x + rndab(-radius,radius),
			dy = Player.pos.y - alien.y + rndab(-radius,radius),
			r = sq(dx)+sq(dy),
			r2 = sq(radius)
			
			if (rnd() < ALIEN_SHOOT_CHANCE*dt*(r<40*r2 ? 3 : 1)   && alienShots.length<MAX_ALIEN_SHOTS) {
				if (r > 99*r2) {
...
//				i: y*12+x,
	...
	range(3, function(j) {
		var color = {
			finishColor: [50, 50, 50, .2],
			finishColorRandom: [10,10,10,0],
			startColor: [220, 225, 225, .9],
			startColorRandom: [5, 5, 5, 0]
		};
		color.finishColor[j] = 140;
